...
- **Player:** `id`, `name`, `image`, `tribe`, `tribeOrder`, `allianceColor`, `status` (`"active" | "eliminated" | "jury"`), `advantages` (`AdvantageId[]`, max 3), `isFavorite` (boolean, optional for migration).
...
- **Store actions:** `setPlayers`, `setViewMode`, `setPlayerAllianceColor`, `setPlayerStatus`, `addPlayerAdvantage`, `removePlayerAdvantages`, `setPlayerFavorite`.
...
- **PlayerCard** (`src/components/PlayerCard.tsx`): Single player card. Uses `useSortable` when used in a column (receives ref, style, listeners, `isDragging`). Shows avatar (with optional face-focus zoom and pan left/right for specific players), name, advantage slots (or placeholders), alliance strip if `allianceColor`, menu (•••). Menu: Set/Remove Alliance, Add/Remove Favorite (gold star icon; toggles `isFavorite`), Add Advantage (if &lt; 3), Remove Advantages (if any), Eliminate / Eliminate to Jury (if active), Resurrect (if not active). Favorite cards: permanent golden outline and subtle gold glow. Eliminated/jury cards: reduced opacity, grayscale avatar, red X overlay, no drag. Portal for dropdown so it renders above other cards (high z-index). Menus for jury/eliminated cards open upward (`openMenuUpward`) so they are not cut off at the bottom of the window.
